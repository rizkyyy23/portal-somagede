...
export const getAllConfigs = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM api_configurations ORDER BY id ASC');
    res.json({ success: true, data: rows });
  } catch (error) {
    console.error('Error fetching API configs:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch API configurations' });
  }
};
...
export const createConfig = async (req, res) => {
  try {
    const { name, endpoint, api_key, description, method, timeout, retry_attempts, status } = req.body;
    
    if (!name || !endpoint || !api_key) {
      return res.status(400).json({ success: false, message: 'Name, Endpoint, and API Key are required' });
    }

    const [result] = await pool.query(
      `INSERT INTO api_configurations (name, endpoint, api_key, description, method, timeout, retry_attempts, status, last_sync) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [name, endpoint, api_key, description, method || 'GET', timeout || 30000, retry_attempts || 3, status || 'active']
    );
  ...
  try {
    ...
    await pool.query(
      `UPDATE api_configurations 
       ...
       WHERE id=?`,
      [name, endpoint, api_key, description, method, timeout, retry_attempts, status, id]
    );
...
export const testConnection = async (req, res) => {
  try {
    const { id } = req.params;
    const [rows] = await pool.query('SELECT * FROM api_configurations WHERE id = ?', [id]);
    
    if (rows.length === 0) return res.status(404).json({ success: false, message: 'Config not found' });

    const config = rows[0];
    
    // In a real scenario, we would use axios/fetch to call config.endpoint here.
