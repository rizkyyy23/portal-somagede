...
export const getAllSessions = async (req, res) => {
  try {
    const [sessions] = await db.query('SELECT * FROM active_sessions ORDER BY login_at DESC');
...
export const createSession = async (req, res) => {
  try {
    const { user_id, user_name, user_email, department, role, ip_address, app_name } = req.body;

    const [result] = await db.query(
      'INSERT INTO active_sessions (user_id, user_name, user_email, department, role, ip_address, app_name) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [user_id || null, user_name, user_email || '', department || '', role || 'User', ip_address || '0.0.0.0', app_name || 'Portal']
    );
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    // Total active users
    const [totalActive] = await db.query("SELECT COUNT(*) as count FROM users WHERE status = 'active'");

    // Total users (all statuses)
    const [totalUsers] = await db.query("SELECT COUNT(*) as count FROM users");

    // Total active applications (assuming all in DB are active)
    ...
    const [sessionCount] = await db.query('SELECT COUNT(*) as count FROM active_sessions');
    ...
    const [recentSessions] = await db.query('SELECT * FROM active_sessions ORDER BY login_at DESC LIMIT 3');
