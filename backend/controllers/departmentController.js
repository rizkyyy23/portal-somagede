...
export const getAllDepartmentPermissions = async (req, res) => {
  try {
    const [rows] = await pool.query(
      "SELECT id, name, code, allowed_apps FROM departments ORDER BY name ASC",
    );
    ...
    const formattedData = rows.map((dept) => {
      ...
      const permissions = apps.map((appCode) => ({
        application_code:
          typeof appCode === "string" ? appCode.trim() : appCode,
        enabled: true,
      }));
...
export const updateDepartmentPermissions = async (req, res) => {
  try {
    ...
    await pool.query("UPDATE departments SET allowed_apps = ? WHERE id = ?", [
      allowed_apps,
      departmentId,
    ]);
    ...
    res
      ...
      .json({
        success: false,
        message: "Error updating department permissions",
        error: error.message,
      });
  }
};
...
export const toggleDepartmentPermission = async (req, res) => {
  try {
    ...
    // Fetch current apps
    const [rows] = await pool.query(
      "SELECT allowed_apps FROM departments WHERE id = ?",
      [departmentId],
    );
    if (rows.length === 0)
      return res
    ...
    // applicationId is the app CODE (e.g. "OODO", "PUNCH")
    const appCode = String(applicationId).trim();
    const index = appsArray.indexOf(appCode);

    if (enabled) {
      // Add if not already present
      if (index === -1) {
        appsArray.push(appCode);
      }
    } else {
      // Remove if present
      if (index > -1) {
    ...
    res
      ...
      .json({
        success: false,
        message: "Error toggling department permission",
        error: error.message,
      });
  }
};
