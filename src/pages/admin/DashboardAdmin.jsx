...
const DashboardAdmin = () => {
  ...
  const confirmForceLogout = async () => {
    ...
    try {
      await fetch(`${API_URL}/sessions/${logoutCandidate.id}`, { method: 'DELETE' });
      fetchDashboardData();
      showToast(`${logoutCandidate.user_name} has been logged out successfully.`, "success");
  ...
  const getDeptType = (dept) => {
    const types = { 'Finance': 'finance', 'IT Department': 'it', 'Warehouse': 'warehouse', 'Human Resources': 'hr' };
    ...
    <div>
      ...
      <div className="dashboard-widgets-grid">
        ...
        <div className="widget-card">
          <div className="widget-header">
            <span className="text-secondary"><Megaphone size={20} /></span>
          ...
          <div className="broadcast-list">
            {activeBroadcasts.length === 0 ? (
              ...
              activeBroadcasts.map((broadcast) => {
                ...
                return (
                  <div key={broadcast.id} className={`broadcast-item ${isUrgent ? 'urgent' : 'normal'}`}>
                    <div className="broadcast-item-header">
                      <strong className="broadcast-item-title">{broadcast.title}</strong>
                      {broadcast.priority === 'urgent' && <span className="urgent-pill">URGENT</span>}
                    </div>
          ...
          <div className="chart-container futuristic-chart">
            {loginTrends.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={loginTrends} margin={{ top: 20, right: 10, left: 0, bottom: 0 }}>
                  <defs>
        ...
        <div className="widget-card">
          <div className="widget-header">
            <span className="text-warning"><Monitor size={20} /></span>
            ...
            {appUsage.length === 0 ? (
              ...
              appUsage.map((app, index) => (
                <div key={index} className="app-usage-item">
                  <div className="widget-header-title">
                    <div className="app-rank-box" style={{ color: COLORS[index % COLORS.length] }}>
                      {index + 1}
                    </div>
        ...
        <div className="widget-card">
          <div className="widget-header-between">
            <div className="widget-header-title">
              <span className="text-danger"><Activity size={20} /></span>
              ...
              <div className="empty-widget-state">
                No users online
                 ...
                 activeDeptStats.slice(0, 4).map((dept, index) => {
                    const totalActive = activeDeptStats.reduce((acc, curr) => acc + curr.count, 0) || 1;
                    ...
                    return (
                        <div key={index}>
                            <div className="dist-item-header">
                                ...
                                <span className="dist-value">{dept.count} <span className="text-small-gray">Active</span></span>
                            </div>
        ...
        <div className="widget-card span-2">
          <div className="widget-header-between">
            <div className="widget-header-title">
              <span className="text-green"><BarChart3 size={20} /></span>
          ...
          <div className="pie-chart-wrapper">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={departmentStats.map(d => ({ name: d.department, value: d.count }))}
                  innerRadius={50}
                  outerRadius={75}
                  paddingAngle={5}
                  dataKey="value"
            ...
            <div className="pie-center-label">
              <div className="pie-center-value">{departmentStats.reduce((a, b) => a + b.count, 0)}</div>
      ...
      {showDeptModal && (
        <div className="modal-overlay" onClick={() => setShowDeptModal(false)}>
          <div className="modal-container dashboard-modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
               <div className="modal-dist-list">
                {departmentStats
                  ...
                  .map((dept, index) => {
                    const totalActiveInDepts = departmentStats.reduce((acc, curr) => acc + curr.count, 0) || 1;
                    const percentage = Math.round((dept.count / totalActiveInDepts) * 100);
                    const color = getDeptColor(dept.department);
                    ...
                    return (
                        <div key={index}>
                            <div className="modal-dist-item-header">
                                <span className="dist-label">{dept.department}</span>
                                <span className="dist-value">{dept.count} <span className="text-small-gray">({percentage}%)</span></span>
                            </div>
                            <div className="progress-bar-bg">
                                <div className="progress-bar-fill" style={{ width: `${percentage}%`, background: color }}></div>
                            </div>
      ...
      {showFullActiveModal && (
        <div className="modal-overlay" onClick={() => setShowFullActiveModal(false)}>
          <div className="modal-container dashboard-modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <div className="widget-header-title">
                <span className="text-danger"><Activity size={20} /></span>
                ...
                {activeDeptStats.map((dept, index) => {
                    ...
                    return (
                        <div key={index}>
                            <div className="modal-dist-item-header">
                                <span className="dist-label">{dept.department}</span>
                                <span className="dist-value">{dept.count} <span className="text-small-gray">Active ({percentage}%)</span></span>
                            </div>
                            <div className="progress-bar-bg">
                                <div className="progress-bar-fill" style={{ width: `${percentage}%`, background: color }}></div>
                            </div>
      ...
      <div className="session-section">
                ...
                activeSessions.map((session) => {
                  ...
                  return (
                    <tr key={session.id}>
                      ...
                      <td>
                        <span className={`role-badge ${(session.role || 'user').toLowerCase()}`}>
                          {session.role || 'User'}
                        </span>
                      </td>
                      <td>
                      ...
                      <td>{formatTime(session.login_at)} ({formatDuration(session.login_at)})</td>
                      <td>
                        <button 
        ...
        <div className="table-footer">
          <span>Showing {activeSessions.length} of {totalSessions} active sessions</span>
          <button 
      ...
      {logoutCandidate && (
        <div className="modal-overlay confirmation-modal" onClick={() => setLogoutCandidate(null)}>
          <div className="modal-container" onClick={(e) => e.stopPropagation()}>
            <div className="modal-body">
              ...
              <p>
                Are you sure you want to force logout <strong>{logoutCandidate.user_name}</strong>? 
                They will be disconnected immediately holding IP <strong>{logoutCandidate.ip_address}</strong>.
              </p>
            </div>
